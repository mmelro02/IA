...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        ...
        for key in self.agent.pairs:
            p = self.agent.pairs[key]
    ...
    def run(self):
        ...
        for step in range(steps - 1):
            ...
            for j in range(len(forklift_path)):
                ...
                if step + 1 < len(forklift_path[j]) - 1 and forklift_path[j][step] == forklift_path[j][step + 1]:
                    if old_cell[j].column > 0 and self.state.matrix[old_cell[j].line][old_cell[j].column - 1] == constants.PRODUCT:
                        self.state.matrix[old_cell[j].line][old_cell[j].column - 1] = constants.PRODUCT_CATCH
                    if old_cell[j].column < self.state.columns - 1 and self.state.matrix[old_cell[j].line][old_cell[j].column + 1] == constants.PRODUCT:
                        self.state.matrix[old_cell[j].line][old_cell[j].column + 1] = constants.PRODUCT_CATCH
                if steps == len(forklift_path[j]) and forklift_path[j][step] == forklift_path[j][step + 1]:
                    dec_step += 1
            self.state.matrix[self.gui.agent_search.exit.line][self.gui.agent_search.exit.column] = constants.EXIT
            self.gui.queue.put((copy.deepcopy(self.state), step - dec_step, False))
        self.gui.queue.put((None, steps - dec_step, True))  # Done
